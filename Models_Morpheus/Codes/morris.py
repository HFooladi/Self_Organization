...
def circle_to_rect(center, radius, Image):
  ...
  while (theta <= 359):
    ...
    polRow = 0
    while r <= 1:
      polarImg[polRow, polCol] = Image[np.int16((1.0 - r) * yptet + r * yitet),
                                       np.int16((1.0 - r) * xptet + r * xitet)]
...
def radial_plot(fileName):
  ...
  field_cur = np.reshape(field[field[:, 0] == lastTime, -1],
                         (n_pixel_x, n_pixel_y))
...
if is_analyze:
  ...
  saved_param_values[:, 0] = np.loadtxt("a1.txt",
                                        dtype=np.float32,
                                        delimiter=';')
  saved_param_values[:, 1] = np.loadtxt("a2.txt",
                                        dtype=np.float16,
                                        delimiter=';')
  ...
  saved_param_values[:, 4] = np.loadtxt("lambda.txt",
                                        dtype=np.float16,
                                        delimiter=';')
  saved_param_values[:, 5] = np.loadtxt("r1.txt",
                                        dtype=np.float16,
                                        delimiter=';')
  ...
  for run in runs_index:
    print run
    if os.path.isdir(os.path.join(sweep_file_name,
                                  "Model_new_test_" + str(run))):
      f1, r1 = radial_plot(
          os.path.join(sweep_file_name, "Model_new_test_" + str(run),
                       "BMP.txt"))
      f2, r2 = radial_plot(
          os.path.join(sweep_file_name, "Model_new_test_" + str(run),
                       "Nog.txt"))
