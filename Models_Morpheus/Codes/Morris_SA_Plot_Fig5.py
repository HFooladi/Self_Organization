...
problem = {
 'num_vars': 7,
 'names': [r'$\tilde{a}_B$', r'$\tilde{a}_N$', r'$\tilde{\beta}_B$', r'$\tilde{\beta}_N$', r'$\lambda$', r'$r_1$', r'$r_2$'],
 'groups': None,
 'bounds': [[0.01, 2],
            [0.01, 2],
            [5, 40],
...
r_Nog = np.loadtxt(os.path.join("sweep_array_all_morris", "r_Nog.txt"), dtype=np.float32)
r_BMP = np.loadtxt(os.path.join("sweep_array_all_morris", "r_BMP.txt"), dtype=np.float32)
f_BMP = np.loadtxt(os.path.join("sweep_array_all_morris", "f_BMP.txt"), dtype=np.float32)
f_Nog = np.loadtxt(os.path.join("sweep_array_all_morris", "f_Nog.txt"), dtype=np.float32)

Y = (np.max(f_BMP, axis=1) - np.min(f_BMP, axis=1)) / (np.argmax(f_BMP, axis=1) - np.argmin(f_BMP, axis=1))

Si = morris.analyze(problem, saved_param_values, Y, conf_level=0.95,
